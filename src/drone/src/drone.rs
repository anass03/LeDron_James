...
// Cache struct
struct Cache{
    history_floodreq: HashMap<NodeId, u64>,
    crashed: bool
}
            ...
            crossbeam_channel::select_biased! { // Prioritizing Controller messages using select_biased! macro.
                recv(self.controller_recv) -> command => {
                    if let Ok(command) = command {
                        ...
                        println!("Drone ID {:?} - Failed to parse command-packet [Crossbeam Error - Channel Closed / Empty]", self.id);
                    }
                }
                ...
                /*
                default => {
                    println!("LeDron ID {:?} - Waiting for messages...", self.id);
...
impl Drone {
    fn log<S: AsRef<str>>(&self, message: S) {
        println!("Drone ID {} - {}", self.id, message.as_ref());
    }
        ...
        if self.cache.crashed{
            if !self.packet_recv.is_empty(){
                ...
                if let Ok(packet) = packet {
                    ...
                    println!("Failed to parse packet [Crossbeam Error]");
                }
            }
        }else{
            if let Ok(packet) = packet {
                ...
                println!("Failed to parse packet [Channel Closed / Empty]");
            }
        }
    }
    ...
    fn handle_flooding_req(&mut self, mut packet_id: FloodRequest, packet: Packet){
        ...
        if self.cache.history_floodreq.iter().any(|(k,v)| *v == packet_id.flood_id && *k == packet_id.initiator_id) {
            ...
            self.cache.history_floodreq.insert(packet_id.initiator_id.clone(), packet_id.flood_id.clone()); // **
            let mut return_packet = Packet{
    ...
    fn build_packet_flood_response(mut flreq_header: FloodRequest, srcid:u64) -> Packet{
